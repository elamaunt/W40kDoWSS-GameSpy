    ...
    public class FaqPageController : FrameController<FAQPageViewModel>
    {
        protected override void OnBind()
        {
            Frame.Questions.DataSource = new ObservableCollection<QuestionItemViewModel>()
            {
                ...
                new QuestionItemViewModel("Исправлены ли баги официальной версии игры?", "Исправлены пока только баги, никак не связанные с балансом игры. Для игры используется специальный мод, который со временем будет улучшаться. Изначально в этот мод уже внедрен другой мод Bugfix версии 1.56a. Багов Steam версии в версии 1.2 нет."),
                ...
                new QuestionItemViewModel("Могу ли я как-то поддержать проект?", @"Вы очень поможете проекту, если расскажите о нем другим людям. Если Вы хотите поддержать проект финансово, то Вы можете использовать следующие реквитизы:
